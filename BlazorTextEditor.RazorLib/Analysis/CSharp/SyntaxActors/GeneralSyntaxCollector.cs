...
public class GeneralSyntaxCollector : CSharpSyntaxWalker
{
    ...
    {
        if (token.Kind().ToString().EndsWith("Keyword")) KeywordSyntaxTokens.Add(token);
    ...
    {
        if (node.IsKind(SyntaxKind.StringLiteralExpression)) StringLiteralExpressionSyntaxes.Add(node);
    ...
    {
        if (node.Declaration.Type.IsVar) VarTextSpans.Add(node.Declaration.Type.Span);
